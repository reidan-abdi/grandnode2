...
public class CurrencyController : BaseAdminController
{
    ...
    public async Task<IActionResult> List(bool liveRates = false)
    {
        if (liveRates)
            try
            {
                var primaryExchangeCurrency =
                    await _currencyService.GetCurrencyById(_currencySettings.PrimaryExchangeRateCurrencyId);
        ...
        foreach (var erp in _exchangeRateService.LoadAllExchangeRateProviders())
            ViewBag.ExchangeRateProviders.Add(new SelectListItem {
                ...
                Selected = erp.SystemName.Equals(_currencySettings.ActiveExchangeRateProviderSystemName,
