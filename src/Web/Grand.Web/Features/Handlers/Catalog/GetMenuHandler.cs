...
public class GetMenuHandler : IRequestHandler<GetMenu, MenuModel>
{
        ...
        bool loadSubCategories = true)
    {
        ...
        var categories = allCategories.Where(c => c.ParentCategoryId == rootCategoryId).OrderBy(x => x.DisplayOrder)
            .ToList();
        foreach (var category in categories)
        {
            ...
            {
                var subCategories = await PrepareCategorySimpleModels(language, allCategories, category.Id);
